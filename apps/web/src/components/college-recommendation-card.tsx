...
function RatingRing({ rating }: { rating: number }) {
  ...
  const finalOffset = RING_CIRC - fraction * RING_CIRC;
    ...
    <div className="flex flex-col items-center gap-1.5">
      <div className="relative h-[76px] w-[76px]">
        ...
        <div className="absolute inset-0 flex items-center justify-center font-fraunces text-[19px] font-semibold text-foreground">
          {rating.toFixed(1)}
          <sup className="ml-px text-[11px] font-medium">/5</sup>
        </div>
      </div>
      <div className="text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground/70">Rating</div>
    </div>
  );
}
...
export function CollegeRecommendationCard({
  ...
  return (
    <article className="grid grid-cols-[72px_1fr] gap-x-4 gap-y-3 rounded-[20px] border border-border bg-card p-[18px_18px_16px] shadow-[0_1px_2px_rgba(24,35,51,0.04),0_8px_24px_rgba(24,35,51,0.06)] min-[900px]:grid-cols-[84px_220px_1fr_1fr_1fr_168px] min-[900px]:items-center min-[900px]:gap-x-5 min-[900px]:p-[22px_26px]">
      <div className="row-span-3 flex flex-col items-center gap-1.5 pt-0.5 min-[900px]:row-auto">
        <RatingRing rating={college.avgRating} />
      </div>
      ...
      <div className="col-span-2 flex flex-wrap gap-2 min-[900px]:col-span-1 min-[900px]:flex-col min-[900px]:gap-2">
        <Button
          ...
          className="flex-1 gap-1.5 min-[900px]:flex-none"
          onClick={() => toggle(college.slug)}
        >
          {shortlisted ? <BookmarkCheck className="h-4 w-4" /> : <Bookmark className="h-4 w-4" />}
          {shortlisted ? 'Shortlisted' : 'Shortlist'}
        </Button>
        <Button size="sm" variant="outline" className="flex-1 min-[900px]:flex-none" onClick={onQuiz}>
          Ask Expert Guide
        </Button>
      </div>
    </article>
