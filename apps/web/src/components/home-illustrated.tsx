...
function TestimonialCard({ t }: { t: (typeof TESTIMONIALS)[number] }) {
  return (
    <article className="flex flex-col gap-5 rounded-[22px] border border-border bg-card p-8 shadow-sm">
      <span aria-hidden className="font-display text-6xl font-semibold leading-[.4] text-primary">“</span>
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="mb-12 text-center font-display text-[clamp(30px,4.4vw,44px)] font-semibold leading-[1.1] sm:mb-14">
      {children}
    </h2>
  );
}
