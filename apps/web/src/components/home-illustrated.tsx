...
function TestimonialCard({ t }: { t: (typeof TESTIMONIALS)[number] }) {
  return (
    <article className="flex flex-col gap-5 rounded-[22px] border border-border bg-card p-8 shadow-sm">
      <span aria-hidden className="font-accent text-6xl font-semibold italic leading-[.4] text-primary">“</span>
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="mb-12 text-center font-accent text-[clamp(30px,4.4vw,44px)] font-semibold italic leading-[1.1] sm:mb-14">
      {children}
    </h2>
  );
}
        ...
        >
          <div className="mx-auto grid grid-cols-1 lg:grid-cols-2 lg:max-w-[1440px]">
            ...
            <div className="relative flex flex-col justify-center px-6 pb-8 pt-8 text-center sm:px-10 sm:pb-10 sm:pt-10 lg:items-start lg:justify-center lg:px-14 lg:pb-0 lg:text-left xl:items-center xl:text-center [@media(max-height:700px)]:py-4">
              <span className="block font-accent text-[clamp(18px,2.4vw,24px)] font-semibold italic text-primary [@media(max-height:700px)]:text-lg">
                India&rsquo;s Student
              </span>
              <h1 className="mt-1 font-accent text-[clamp(28px,4vw,42px)] font-semibold italic leading-[1.15] [@media(max-height:700px)]:mt-0.5 [@media(max-height:700px)]:text-[clamp(24px,3.6vw,34px)]">
                Success Network
              </h1>
